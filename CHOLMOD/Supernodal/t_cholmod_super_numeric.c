...
static void * TEMPLATE (cholmod_super_numeric_threaded) (void *void_args)
{
    ...
    int *cpu_free_p, useGPU, device, vdevice;
    cholmod_gpu_pointers *gpu_p ;
#endif
...
#ifdef GPU_BLAS
    cpu_free_p = thread_args->cpu_free_p;
...
#pragma omp critical
        ...
        if (info != 0)
        {
            ...
            else
            {
                ...
                TEMPLATE (cholmod_super_numeric_threaded) (void_args);
    ...
    )
{
    ...
    int device, vdevice;
#ifdef GPU_BLAS
    int cpu_free = CPU_THREAD_NUM,  useGPU = FALSE;
