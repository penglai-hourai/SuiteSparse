...
static int read_header_batched	/* returns TRUE if successful, FALSE on error */
(
 size_t offset[],
    /* ---- input ---- */
    FILE *f[],		/* file to read from */
...
{
    char *p ;
    int first, got_mm_header = FALSE, c, c2, is_complex, nitems ;
    double l1, l2, l3, l4 ;
    ...
    *stype = STYPE_UNKNOWN ;
    ...
    for (file_itr = 0; file_itr < NMATRICES; file_itr++)
    {
        file = f[file_itr];
        if (file == NULL)
            continue;
...
static cholmod_triplet *read_triplet_batched
(
 size_t offset[],
    /* ---- input ---- */
    FILE *f[],		    /* file to read from, must already be open */
...
)
{
	...
	for ( ; ; )
	{
	    if (file_itr >= NMATRICES)
	    {
		ERROR (CHOLMOD_INVALID, "matrix index overflow") ;
		return (NULL) ;
	    }
    file = f[file_itr];
	    if (file == NULL)
	    {
		ERROR (CHOLMOD_INVALID, "matrix not found") ;
		return (NULL) ;
	    }
	...
	nitems = (nitems == EOF) ? 0 : nitems ;
	i = l1 + offset[file_itr] ;
	j = l2 + offset[file_itr] ;
...
cholmod_triplet *CHOLMOD(read_triplet_batched)
(
    /* ---- input ---- */
    FILE *f[],		/* file to read from, must already be open */
    /* --------------- */
...
)
{
    ...
    ncol = 0;
    nnz = 0;
    if (!read_header_batched (offset, f, buf, &mtype, &nrow, &ncol, &nnz, &stype) ||
	mtype != CHOLMOD_TRIPLET)
    {
	/* invalid matrix - this function can only read in a triplet matrix */
	ERROR (CHOLMOD_INVALID, "invalid format") ;
    ...
    return (read_triplet_batched (offset, f, nrow, ncol, nnz, stype, FALSE, buf, Common)) ;
}
...
cholmod_sparse *CHOLMOD(read_sparse_batched)
(
    /* ---- input ---- */
    FILE *f[],		/* file to read from, must already be open */
    /* --------------- */
...
)
{
    ...
    T = CHOLMOD(read_triplet_batched) (f, Common) ;
    A = CHOLMOD(triplet_to_sparse) (T, 0, Common) ;
    CHOLMOD(free_triplet) (&T, Common) ;
