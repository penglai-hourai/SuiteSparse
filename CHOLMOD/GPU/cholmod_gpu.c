...
)
{
    ...
    for(k = 0; k < Common->numGPU; k++)
    {
      ...
      for(j = 0; j < CHOLMOD_DEVICE_LX_BUFFERS; j++)
      {
        ...
        if (Common->updateCDevBuffersFree[k][j])
        {
            cudaEventDestroy (Common->updateCDevBuffersFree[k][j]) ;
            Common->updateCDevBuffersFree[k][j] = NULL ;
        }
...
/* only if compiled with GPU */
    ...
    requestedHostMemory = (size_t) (requestedDeviceMemory
        * (CHOLMOD_HOST_SUPERNODE_BUFFERS+1)
    ...
    Common->devBuffSize = (size_t) (requestedDeviceMemory / (CHOLMOD_DEVICE_SUPERNODE_BUFFERS * Common->numGPU_parallel));
