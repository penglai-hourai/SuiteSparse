 ...
 *      3. addUpdate (map schur comp. to supernode)
 */
int TEMPLATE2 (CHOLMOD (gpu_updateC_factorized))
  (
   cholmod_common *Common,
   cholmod_gpu_pointers *gpu_p,
   double *Lx,
...
#pragma omp parallel for num_threads(numThreads) private (icol, irow) if (ndcol > 32)
  for ( icol=0; icol<ndcol; icol++ ) {
    for ( irow=0; irow<ndrow2; irow++ ) {
      gpu_p->h_Lx_root[gpuid][iHostBuff][icol*ndrow2+irow] =
        Lx[pdx1 + icol*ndrow + irow];
    }
  }
  ...
  cudaErr = cudaMemcpyAsync (
          ...
          ndrow2*ndcol*sizeof(devPtrLx[0]),
  ...
  if (ndrow3 > 0)
  {
    ...
    cublasStatus = cublasDgemm (
            ...
            &alpha,                         	/* ALPHA:  1 */
            devPtrLx + ndrow1,    	/* A, LDA: L2*/
            ndrow2,                         	/* ndrow */
            devPtrLx,                       	/* B, LDB: L1 */
            ndrow2,                         	/* ndrow */
            &beta,                          	/* BETA:   0 */
            devPtrC + ndrow1,       	/* C, LDC: C2 */
            ndrow2);
#else
